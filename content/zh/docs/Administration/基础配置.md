...
| 内核启动参数             | 描述                                        | 默认值           | 备注                                                         |
|--------------------|-------------------------------------------|---------------|------------------------------------------------------------|
| crashkernel=x      | 在4G一下的物理内存预留X大小的内存给kdump使用。               | x86版本默认配置512M | 该配置方法只在4G以下内存预留，必须保证4G一下连续可用内存足够预留。                        |
...
### 预留内存推荐值
#### 推荐方案一
crashkernel=2048M,high
说明：在4G以下预留256M，在4G以上预留2048M，共2304M。

#### 推荐方案二
内存<=512G，且不通过网络转储kdump文件时：crashkernel=1024M,high
内存>512G时：crashkernel=2048M,high
说明：不通过网络转储时，需要设置kdump文件系统不打包网络相关驱动。网络驱动加载需要申请较大内存，可能导致预留内存不足，kdump失败。因此建议禁用网络相关驱动。


### 禁用网络相关驱动
kdump配置文件（/etc/kdump.conf）中，dracut参数可以设置裁剪的驱动模块，可以将网络驱动配置到裁剪驱动列表中，让kdump文件系统中不加载该驱动。
