...
默认情况下 IMA 不启用，用户可以通过在启动参数中设置 `integrity=1` 后重启系统打开 IMA 度量，此时系统会前往 `/etc/ima/` 路径下寻找 ima-policy 策略文件，并按照策略在启动时度量系统中的文件。如果不想手动编写策略文件，也可以在启动参数中配置 `ima_policy=tcb` 使用默认策略（更多策略参数请参考附录“IMA启动参数”章节）。

系统当前加载的 IMA 策略可以在 `/sys/kernel/security/ima/policy` 文件中查看，IMA 度量日志则位于`/sys/kernel/security/ima/ascii_runtime_measurements` 文件中：
...
* IMA measurement：
  ...
  ```shell
  ima_policy=exec_tcb ima_digest_list_pcr=11 evm=ignore integrity=1
  ```

* IMA appraisal 日志模式 + IMA measurement：
...
* IMA appraisal 强制模式 + IMA measurement：

  ```shell
  ima_appraise=enforce-evm evm=ignore ima_appraise_digest_list=digest ima_digest_list_pcr=+11 ima_template=ima-sig ima_policy="exec_tcb|appraise_exec_tcb|appraise_exec_immutable" initramtmpfs integrity=1
  ```
...
```shell
ima_appraise=fix ima_policy=appraise_tcb integrity=1
```

`fix` 模式会允许系统在没有参考值的情况下启动，`appraise_tcb` 对应了一种 IMA 策略，具体可参考附录中的“IMA 启动参数”章节，`integrity=1` 是 IMA 特性的总开关。
...
```shell
ima_appraise=enforce ima_policy=appraise_tcb integrity=1
```
...
2. 标记扩展属性。
   ...
   在标记之前，请确认环境中已安装 digest-list-tools 和 ima-evm-utils，且版本不低于：
   ...
   为系统中已安装的软件标记 securtiy.ima 和 security.evm 扩展属性：

   ```shell
   $ manage_digest_lists -p add-ima-xattr -d /etc/ima/digest_lists.tlv
   $ manage_digest_lists -p add-evm-xattr -d /etc/ima/digest_lists.tlv
   ```


3. 执行 `dracut` 重新生成 initrd：
...
IMA 评估 enforce 模式下，被篡改内容的文件无法执行，文件扩展属性将拒绝修改：

![](./figures/ima_demo.png)
