...
使用devicemapper作为graphdriver时，强杀可能导致信号量残留。docker在操作dm的过程中会创建信号量，如果在释放信号量前，daemon被强杀，可能导致该信号量无法释放，一次强杀最多泄露一个信号量，泄露概率低。而linux系统有信号量上限限制，EulerOS上默认128，当信号量泄露次数达到128将无法创建新的信号量，进而导致docker daemon启动失败。排查方法如下：
