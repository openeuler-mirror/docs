...
在安装开始前，您需要获取 openEuler 树莓派版本的镜像和校验文件。

当前仅提供 openEuler 20.03 LTS aarch64 的内测版本镜像，下载地址：<https://isrc.iscas.ac.cn/eulixos/repo/others/openeuler-raspberrypi/images>，其中包括镜像及其校验文件。

1. 下载镜像
    为了快速下载，建议选择压缩版本，即点击 “openEuler-20.03-LTS-aarch64-raspi.img.xz”，将 openEuler 树莓派版本镜像的压缩包下载到本地。

2. 下载校验文件
    点击 “openEuler-20.03-LTS-aarch64-raspi.img.xz.sha256sum”，将该压缩包的校验文件下载到本地。
...
镜像文件：openEuler-20.03-LTS-aarch64-raspi.img.xz

校验文件：openEuler-20.03-LTS-aarch64-raspi.img.xz.sha256sum
...
1.  获取校验文件中的校验值。执行命令如下：
    ...
    ```
    # cat openEuler-20.03-LTS-aarch64-raspi.img.xz.sha256sum 
    ```

2.  计算文件的 sha256 校验值。执行命令如下：

    ```
    # sha256sum openEuler-20.03-LTS-aarch64-raspi.img.xz
    ```
...
openEuler 树莓派版本镜像所需的最小硬件要求如[表2](#tff48b99c9bf24b84bb602c53229e2542)所示。
