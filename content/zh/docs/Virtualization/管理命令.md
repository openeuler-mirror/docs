...
-   执行虚拟机生命周期操作之前，需要查询虚拟机状态以确定可以执行对应操作。状态之间的基本转换关系请参见“[总体介绍](总体介绍-0.md)”中的“[状态转换](总体介绍-0.md#section157801951173112)”。
-   具备管理员权限。
-   准备好虚拟机XML配置文件。
...
```
virsh <opetate> <obj> <options>
```

各参数含义如下：

-   _opetate_：管理虚拟机生命周期对应操作，例如创建、销毁、启动等。
