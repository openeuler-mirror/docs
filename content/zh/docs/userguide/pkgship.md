...
6. 自编译自安装依赖查询。
    ...
    > 使用场景：如果开发者想在现有的版本库的基础上引入新的软件包，应同时引入该软件包的所有编译、安装依赖。该命令提供开发者一个同时查询这两种依赖关系的功能，能让开发者知晓该软件包会引入那些其他的包。支持查询二进制包或源码包，查询源码包时会将该源码包下的所有二进制包的安装依赖都查询出来，并且支持用户自定义优先级跨库查询。  
    ```
