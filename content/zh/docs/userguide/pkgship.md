...
4. 安装依赖查询。
    ...
    > 使用场景：用户需要安装某个二进制软件包时，需要将该软件包A1的安装依赖B1，及B1的安装依赖C1...直至没有新增的安装依赖全部安装入系统才能成功安装软件包A。因此，在用户安装软件包A之前，可能会需要查询二进制软件包A的所有安装依赖。该命令提供了此功能，该命令允许用户根据平台默认的优先级在多个版本库之间进行查询。    
    ```
...
5. 编译依赖查询。
    ...
    > 用户要编译某个源码包A的时候，需要安装源码包A的编译依赖B1, 要成功安装编译依赖B1需要获取B1的所有安装依赖。因此，在用户编译源码包A之前，可能会需要查询源码包的编译依赖以及这些编译依赖的所有安装依赖。该命令提供了此功能，该命令允许用户根据平台默认的优先级在多个数据库之间进行查询。
    ```
