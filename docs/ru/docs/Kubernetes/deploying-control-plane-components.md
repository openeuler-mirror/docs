# Развертывание компонентов на плоскости управления

## Подготовка файла kubeconfig для всех компонентов
...
## Конфигурация для генерирования поставщика ключей

При запуске api-server необходимо предоставить пару ключей `--encryption-provider-config=/etc/kubernetes/pki/encryption-config.yaml`. В данном документе пара ключей `--encryption-provider-config=/etc/kubernetes/pki/encryption-config.yaml` генерируется с помощью urandom.
...
В данном документе сертификаты, ключи и конфигурации, используемые всеми компонентами, хранятся в каталоге `/etc/kubernetes/pki/`.
...
## Развертывание RBAC роли администратора

Включите роль администратора.
...
## Развертывание сервиса API-сервера

Измените конфигурационный файл etc для apiserver.
...
Все конфигурации apiserver определены в файле `/etc/kubernetes/config` и могут быть использованы непосредственно в сервисном файле.

Большинство конфигураций являются фиксированными. Обратите внимание на следующее:

- Значение параметра `--service-cluster-ip-range` должно совпадать со значением `clusterDNS`.

### Компиляция конфигурации systemd apiserver
...
## Развертывание сервиса controller-manager

Измените конфигурационный файл controller-manager.
...
### Компиляция конфигурационного файла systemd controller-manager
...
## Развертывание сервиса scheduler

Измените конфигурационный файл scheduler.
...
### Компиляция конфигурационного файла systemd scheduler
