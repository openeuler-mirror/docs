...
Если виртуальная память составляет 8 ГиБ, количество vCPU равно 4, режим обработки — custom, модель процессора — Kunpeng-920, а pmull отключен, конфигурация будет следующей:
...
| ----------- | ----------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **disk**    | **type**    | Тип внутреннего устройства хранения.                         | **block**: блочное устройство хранения **file**: файловое устройство хранения **dir**: путь к каталогу<br />**network**: сетевой диск |
...
| -------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| source         | Внутреннее устройство хранения, которое соответствует атрибуту **type** элемента **disk**. | **file**: файловое устройство хранения. Значение представляет собой полностью определенный путь к соответствующему файлу. <br />**dev**: блочное устройство хранения. Значение представляет собой полностью определенный путь к соответствующему устройству хоста. <br />**dir**: тип каталога. Значение представляет собой полностью определенный путь к дисковому каталогу.<br/>**protocol**: используемый протокол.<br/>**name**: Имя диска RBD. Формат: pool/pool/volume .<br/>**host name**: mon-адрес. <br/>**port**: порт mon-адреса.<br/> |
| driver         | Подробная информация об указанном внутреннем драйвере.       | **type**: тип формата диска. Значение — **raw** или **qcow2** — должно совпадать с исходным типом. <br />**io**: режим ввода-вывода диска. Значения: **native** и **threads**. <br />**cache**: режим кэширования диска. Значения: **none**, **writethrough**, **writeback**, **directsync**. <br />**iothread**: поток ввода-вывода, выделенный диску.<br/>**error_policy**: политика обработки при возникновении ошибки записи ввода-вывода. Значением может быть stop, report, ignore, enospace или retry.<br/>**rerror_policy**: политика обработки при возникновении ошибки чтения ввода-вывода. Значением может быть stop, report, ignore, enospac или retry.<br/>**retry_interval**: интервал повторных попыток ввода-вывода. Значение варьируется от 0 до MAX_INT, измеряется в миллисекундах. Данный параметр может быть установлен только в том случае, если для error_policy или rerror_policy установлено значение retry.<br/>**retry_timeout**: интервал тайм-аута повторных попыток ввода-вывода. Значение варьируется от 0 до MAX_INT, измеряется в миллисекундах. Данный параметр может быть установлен только в том случае, если для error_policy или rerror_policy установлено значение retry. |
