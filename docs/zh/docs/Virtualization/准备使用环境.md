...
1.  关闭 SELinux，否则 ovsdb-server Manager 无法正常工作。

    ```
    # setenforce 0
    ```

    查询 SELinux 是否关闭成功，可以参考如下命令和回显。
    ```
...
>启动 Open vSwitch 服务过程中，可以根据需要使用相关服务，其中 ovn-controller-vtep.service 服务必须在普通用户下使用，否则操作会失败。其他服务，需要使用 root 权限。
