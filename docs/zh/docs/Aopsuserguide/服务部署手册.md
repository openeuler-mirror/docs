...
![a-ops软件架构](./images/a-ops软件架构.png)
...
1. 配置yum源：openEuler21.09 和 openEuler21.09:Epol
   ...
   [openEuler21.09]
   name=openEuler21.09
   baseurl=http://119.3.219.20:82/openEuler:/21.09/standard_aarch64/
   enabled=1
   gpgcheck=0
   
   [Epol]
   name=Epol
   baseurl=http://119.3.219.20:82/openEuler:/21.09:/Epol/standard_aarch64/
   enabled=1
   gpgcheck=0
   ```
...
2. 通过yum install 安装
   ...
   yum install aops-databse # A-Ops 数据中心服务：数据存放和数据库管理。
...
```shell
aops-basedatabase elasticserch # 安装21.09源中的mysql数据库并启动
```
...
4. 异常检测配置：
   ...
   1. 异常检测scheduler配置默认配置文件存放在/etc/aops/check_scheduler.ini,请根据实际情况对配置进行更改。详细说明参照异常检测部分。

      ```ini
      [producer]
      kafka_server_list = xx.xx.xx.xx:xxxx #kafka ip 和 端口
      api_version = 0.11.5
      acks = 1
      retries = 3
   ...
   2. 异常检测check_executor配置默认配置文件存放在/etc/aops/check_executor.ini,请根据实际情况对配置进行更改。详细说明参照异常检测部分。
      ...
      ```ini
      [consumer]
      kafka_server_list=90.90.64.64:9092 #kafka ip 和 端口
      group_id=DiagGroup
      enable_auto_commit=False
      auto_offset_reset=earliest
      timeout_ms=5
...
5. 故障诊断配置： 
   ...
   1. 故障诊断diag_scheduler配置默认配置文件存放在/etc/aops/diag_scheduler.ini,请根据实际情况对配置进行更改。
      ...
      [service]
      ip=127.0.0.1 # diag_scheduler ip 和 端口
      port=60116
      ```

      

   2. 故障诊断diag_executor配置默认配置文件存放在/etc/aops/diag_executor.ini,请根据实际情况对配置进行更改。
...
1. **方法一**：vscode转发，使用systemd 启动 web 服务，web服务通过nginx 拉起。
...
2. **方法二**：蓝云部署配置。
   ...
   - 主要修改以下内容
     - web服务端口号，默认为80
     - 后端服务路由地址，需与后端每个服务对应，其中//表示机器管理服务，/domain/host/confs/management表示配置溯源服务，check表示异常检测服务，diag表示诊断服务
   ...
   - 访问
     ...
     假设web服务启动在1.1.1.1:80，则访问1.1.1.1:80即可访问页面（注意浏览器所在的机器与web服务的机器需要能ping通）
   ...
   ![img](file:///C:/Users/c00607941/AppData/Roaming/eSpace_Desktop/UserData/c00607941/imagefiles/7C39D56D-9DAE-448A-AEE2-5781D081B2B2.png)
   ...
   ![img](file:///C:/Users/c00607941/AppData/Roaming/eSpace_Desktop/UserData/c00607941/imagefiles/B93B0EF0-131C-4585-8C50-65D3D37F489C.png)
   ...
   ![img](file:///C:/Users/c00607941/AppData/Roaming/eSpace_Desktop/UserData/c00607941/imagefiles/32AA1DAB-6A3C-4B32-A548-460B52FEA2DF.png)
   ...
   ![img](file:///C:/Users/c00607941/AppData/Roaming/eSpace_Desktop/UserData/c00607941/imagefiles/4FFB9A4D-E32A-4C33-9DB2-5CB5F90C2743.png)
   ...
   ![img](file:///C:/Users/c00607941/AppData/Roaming/eSpace_Desktop/UserData/c00607941/imagefiles/F9665C73-B745-42A8-81E5-A55F280AB49D.png)
   ...
   ![img](file:///C:/Users/c00607941/AppData/Roaming/eSpace_Desktop/UserData/c00607941/imagefiles/DBEEC73E-C7BC-4EA1-B282-DE6C5E4423B2.png)
   ...
   ![img](file:///C:/Users/c00607941/AppData/Roaming/eSpace_Desktop/UserData/c00607941/imagefiles/029B66B9-5A3E-447E-B33C-98B894FC4833.png)
   ...
   ![img](file:///C:/Users/c00607941/AppData/Roaming/eSpace_Desktop/UserData/c00607941/imagefiles/DCC7FE51-4200-49CF-8007-C8EA55E95DAC.png)
...
- check_scheduler.ini 异常检测日志

  功能：负责显示异常检测模块运行日志。

- diag_scheduler.ini 故障诊断日志
...
- /usr/lib/python3.8/site_package/aops_manager/deploy_manager/ansible_handler下的修改：

  - inventory目录下 将相应任务中的主机需要改为执行任务的主机ip，以gala-ragdoll为例：
...
- 任务配置
  ...
  - /usr/lib/python3.8/site_package/aops_manager/deploy_manager/task/95c3e692ff3811ebbcd3a89d3a259eef.yml，根据需求选择部署，设置enable 为true。continue选项为某一步执行失败不阻塞其他任务。
  ...
  - accouont certificate ： key 必须是添加主机时使用的key ， 每次重启manager后必须重新执行certificate
