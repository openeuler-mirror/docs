...
| -drive           | id=rootfs,file=/path/to/rootfs[,readonly=false,direct=true,serial=serial_num,iothread=iothread1,iops=200] | 配置virtio-blk设备                                           |
| -netdev          | id=iface_id,netdev=tap0[,mac=mac_address,iothread=iothread2] | 配置virtio-net设备                                           |
...
| -iothread        | id="iothread1"                                               | 配置iothread的id，这个id可以作为-drive和-netdev的参数        |
| -balloon          | deflate-on-oom=true                                          | 配置balloon设备                                              |
| -mem-path        | /dev/hugepages                                               | 配置内存大页                                                 |
