...
```
## 设置NMI watchdog 
NMI(Non-Maskable Interrupt)：作为系统最高优先级的中断，属于不可屏蔽的中断类型。NMI看门狗可用于检测服务器异常挂起事件，并触发系统收集故障信息，之后触发系统重启恢复。但是强烈建议在启用NMI看门狗之前清楚NMI watchdog的使用方式和由此对系统带来的性能影响。openEuler QA测试团队实验室测试验证结果表明，开启NMI watchdog的情况下，NMI偶尔会产生大量中断，并降低服务器性能。 

本节介绍如何设置NMI watchdog。

kernel.nmi_watchdog = 0 #表示NMI watchdog未开启状态  
kernel.nmi_watchdog = 1 #表示NMI watchdog开启状态
...
### 受限使用说明
- 在某些情况下，HP NMI看门狗不会生成故障转储，hpwdtHP NMI看门狗的驱动程序有时无法响应由HPE看门狗计时器生成的不可屏蔽中断（NMI），因为该perfmon驱动程序会消耗该NMI，导致hpwdt在某些情况下无法调用紧急情况来生成故障转储。 
- 该NMI watchdog设置仅适用openEuler x86_64架构各版本，aarch64架构下的NMI watchdog支持情况及对应设置，请关注后续版本支持情况；