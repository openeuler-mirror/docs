...
- [FAQs](#faqs)
    ...
    - [Fails to Selected Only One Disk for Reinstallation When openEuler Is Installed on a Logical Volume Consisting of Multiple Disks](#fails-to-selected-only-one-disk-for-reinstallation-when-openeuler-is-installed-on-a-logical-volume-consisting-of-multiple-disks)
...
After a disk fails, openEuler fails to be reinstalled because a logical volume group that cannot be activated existed in the system.
...
Check whether the installation source is abnormal.
Use the new installation source.

## Kdump Service Fails to Be Enabled

### Symptom

The following information is displayed after running the  **systemctl status kdump**  command, indicating that no memory is reserved.
...
For the scenario where the OS is installed
...
## Fails to Selected Only One Disk for Reinstallation When openEuler Is Installed on a Logical Volume Consisting of Multiple Disks
...
3.  Run the following command to restart the installation program for the modification to take effect:
    ...
    >![](./public_sys-resources/icon-note.gif) **NOTE:**   
    >You can also press  **Ctrl**+**Alt**+**F6**  to return to the GUI and click  **Refresh**  in the lower right corner to make the storage configuration take effect.  
...
Access the BIOS, set  **secure boot**  to  **disabled**, and reinstall openEuler.
...
PCP-related software package is installed if you select Server > Performance Tool during the OS installation. After the OS is installed and restarted, an error "pmie_check failed in /usr/share/pcp/lib/pmie" is reported in the /var/log/messages file.
...
During the OS installation, the OS is installed on two disks. In this case, if you select one disk for custom partitioning, and click **Cancel** to perform custom partitioning on the other disk, the installation fails.
...
A user selects a disk for partitioning. After the user clicks **Cancel** and then selects the other disk, the disk information is incorrect. As a result, the installation fails.
